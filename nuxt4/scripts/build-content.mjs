...
for(const file of await walk('content')){
 ...
 if(data.updated)post.updated=new Date(data.updated).toISOString();
...
site.article={share:{enable:config.article?.share?.enable === true},lastUpdated:{enable:config.article?.lastUpdated?.enable === true,minimumAgeDays:config.article?.lastUpdated?.minimumAgeDays ?? 90}};
site.license=config.license;
...
site.timeZone = config.site.timeZone || 'Asia/Shanghai';
