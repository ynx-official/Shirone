// Selection is article-scoped; only explicitly synchronized groups persist a preference.
export function initOptionGroups(container: HTMLElement) {
  ...
  function activate(
    ...
    for (let itemIndex = 0; itemIndex < tabItems.length; itemIndex += 1) {
      ...
      // Length parity and index bounds are checked before updating the group.
    ...
    for (const peer of container.querySelectorAll<HTMLElement>(ROOT_SELECTOR)) {
