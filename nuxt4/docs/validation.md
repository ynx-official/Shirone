...
## 2026-09-20 文章详情对照与优化

- 实际启动 Astro 4321 / Nuxt 4322，采集普通 Markdown、代码展示、Markdown 扩展的桌面和手机截图；[原版与修复前后对比](comparison/article/README.md)。
- 新增文章回归先复现封面缺失、代码 padding 覆盖及公式滚动键盘焦点缺失，再修复并验证。标题复制、代码复制不含行号、移动布局、暗色、客户端导航、SSR、禁用 JavaScript 与 axe 均覆盖。
- 13 项 Node 测试、Nuxt 类型检查、ESLint、生产构建通过；最终生产产物在 4323 上运行，10 项相关 Chromium 回归全部通过（8.7 秒），包含 5 项新文章测试、Pagefind、加密切页、生产管理隔离、禁用 JavaScript 和首页 axe。
- 单独验证 Markdown 扩展文章的 updated 元数据及 SSR 最后更新日期为 2024-11-29。
- 开发服务专项检查曾误选两个固定访问 4323 的既有生产测试；当时尚未启动该端口，连接被拒绝。上述最终生产回归已包含并通过这两项，不把先前结果作为成功记录。
- 使用当前 Node 22.12.0 验证，版本警告未修改；只安装旧站缺失依赖和生成忽略资源，没有修改旧站跟踪文件。旧站与临时生产验证进程在检查后停止，原有 Nuxt 开发服务保留。

## 2026-09-20 Windows 开发启动修复

- 在 Windows / Node 22.12.0 上复现了浏览器编译插件返回 `/D:/...` 非绝对路径，以及内容扫描使用反斜杠后目录解析失败的问题；新增回归测试先失败再通过。
- 文件 URL 使用 `fileURLToPath` 转为原生路径；内容标识和 snippet 键统一使用 `/`；图片目录边界使用平台分隔符。契约测试的共享目录别名同步修复。
- `pnpm.cmd test`：13/13 通过；`pnpm.cmd lint`、`pnpm.cmd typecheck` 通过。
- 实际执行 `pnpm.cmd dev`，20 篇文章构建成功、Nuxt/Nitro 启动成功；首页和 `/api/health` 均返回 HTTP 200，首页包含中文 SSR 标记。验证后已停止开发服务。
- 本轮未重跑生产构建及浏览器交互套件。Node 22.12.0 仍低于声明支持范围，启动成功不代表该版本受支持；建议使用 Node 24.11 及以上的 Node 24 版本。
...
- 类型检查、ESLint、生产构建通过。
- 两项语言回归通过（8.3 秒）：中文即时切换、路由/刷新保留、无 JavaScript SSR、独立访客隔离；西班牙语键盘检索、手机/窄桌面布局与顶栏 axe。
- 另七项既有回归通过：首页 axe、桌面/手机布局、主题面板三项、More 菜单。
- 开发热更新在批量语言资源修改时异常，已重启 4322 预览服务。语言功能使用现有轻量翻译层，不引入额外 i18n 依赖或修改公开 URL；文章和配置内容不自动翻译。

## 2026-09-19 默认中文与全站 mock 中文化

- 默认简体中文，首页 SSR、接口、中文全文搜索、图片图注资源、Feed 与旧英文分类链接验证通过；20 篇公开文章 URL 索引和原有语法特征清单保持一致。
- 10 项数据契约/中文初始数据测试通过；TypeScript 与 ESLint 通过。
- 生产浏览器回归分批覆盖 25 个场景：既有交互 23 项首轮通过；修复桑基图节点兼容性和独立访客测试的显式空 Cookie 后，中文专项 3/3、语言切换 2/2 复测通过。中文专项使用独立结果目录，避免并行检查覆盖 Playwright 临时文件。
- Node 24 生产构建及将产物复制到独立目录后的启动验证通过。
- 实际开发浏览器检查首页 `zh-CN`、中文文章管理初始数据、全部 32 项中文设置标题通过；截图见 [中文首页](comparison/chinese-home.png)。
- 开发预览发现共享翻译模块被 Nitro 错误外部化为根路径文件引用，现将 app/i18n 显式内联，重启后健康检查恢复 200。
