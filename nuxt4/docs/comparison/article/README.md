...
按现有 widget 配置区分 top 与 sticky 分区。桌面侧栏拉伸到页框高度，sticky 分区停靠在顶栏下方 5rem，最大高度为视口减去 6rem，超长内容在分区内滚动；目录取消独立滚动层，避免双重滚动。低于 1024px 恢复普通流布局，副栏仍按既有断点隐藏。同步更新分组后的入场动画索引。

桌面/矮屏/手机、页尾边界、目录跳转与站内返回测试通过；生产产物 3 项回归（含无 JavaScript）通过，入场动画与首页 axe 4 项通过，类型检查、ESLint、构建通过。[双侧停靠截图](sidebar-sticky.png)。
...
相册详情恢复原版按图片比例展示的瀑布流和竖图优先排序，接入与原项目同版本 Fancybox 6.1.15 及定制样式，替换简易 dialog。保留缩略图、计数、缩放、旋转、翻转、全屏和播放工具，支持整本相册连续切图，并正确从点击的图片开始。查看器与样式在点击时加载；无 JavaScript 时仍可通过链接打开原图。修正手机预览右侧滚动条留白，退出时恢复滚动和键盘焦点。

开发及生产产物各 3 项浏览器回归通过，覆盖切图、实际缩放、指定图片起始索引、关闭后的焦点、手机全宽与滚动解锁、无 JavaScript 原图链接；类型检查、ESLint、生产构建通过。[相册预览截图](album-viewer-nuxt.png)。

## 2026-09-20 相册详情结构

恢复旧版独立封面卡片与图库分区：封面叠加返回入口、标题、描述、日期、地点、照片数量和标签；桌面封面最小高度 22rem，手机 19rem，图库保留独立背景及内边距。补齐返回与地点图标，沿用现有十语言文案。加密相册在解锁前不显示未知数量。

同时修复纯外链相册因缺少本地图片目录而被编译器忽略的问题。数据契约 9 项与相册浏览器 5 项通过（包括布局、返回导航、手机、预览及 axe），类型检查、ESLint 和生产构建通过。[外链相册结构截图](album-detail-nuxt.png)。
